...
document.addEventListener("submit", async (event) => {
    ...
    if(event.target && event.target.id === "update-teacher-form"){
        event.preventDefault();
        // console.log("Form submitted");
    ...
    //  Form for updating photo
    if(event.target && event.target.id === "update-photo-form"){
        event.preventDefault();
        // console.log("Form submitted");
        ...
        try{
            const res = await fetch("/api/updatePhoto", {
                method: "PATCH",
                headers: {"Content-type": "application/json"},
                body: JSON.stringify(data)
            })
...
document.addEventListener("click", async (event) => {
    ...
    if (event.target.classList.contains("delete-photo")) {
        const photoItem = event.target.closest(".admin_image_container");
        ...
        //  Open popup to handle edit
        if(event.target.classList.contains("edit-photo")){
            const photoItem = event.target.closest(".admin_image_container");
            const photoID = photoItem.getAttribute("data-id");
            // console.log("Now editing photo ", photoID);
            const color = window.getComputedStyle(addButton).getPropertyValue("background-color")
    
            popup.innerHTML = `<div class="section_advertisement-new-text-close-button">X</div>
                        <h2 class="section_advertisement-new_text_name">${buttonsBackgrounds[color]}</h2>
                        <form id="update-photo-form" class="user-form">
                                <fieldset>
                                    <label for="photoID">photoID</label>
                                    <input type="text" id="photoID" name="photoID" placeholder="photoID" readonly autocomplete="photoID" value="${photoID}">
                                    ...
                                    <label for="alt_text">Альтернативний текст</label>
                                    <input type="text" id="alt_text" name="alt_text" placeholder="Введіть альтернативний текст" required autocomplete="alt_text">
                                    <button type="submit" class="btn-submit">Оновити</button>
                                </fieldset>
                            </form>`
